...
1. Define `GetUserOrganisationsRequest` / `GetUserOrganisationsResponse` in `Core.Contracts/PublicApi/`
2. Implement `GetUserOrganisationsUseCase` — dispatch `GetOrganisationsAssociatedWithUserRequest`, filter `Status == OrganisationStatus.Hidden` (value 0), throw `UserNotFoundException` if empty
3. Register in `PublicApi/Configuration/` and add `GET users/{userId}/organisations` endpoint
...
| 2 | DSI-8679 — Get organisations for user | Simple; uses existing NodeApi requester |
