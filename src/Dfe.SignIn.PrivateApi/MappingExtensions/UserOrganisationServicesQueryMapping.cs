...
namespace Dfe.SignIn.PrivateApi.MappingExtensions;

/// <summary>
/// Map GetUserOrganisationService to data transfer objects.
/// </summary>
public static class UserOrganisationServicesQueryMapping
{
    /// <summary>
    /// Trasform to users.
    /// </summary>
    ...
    /// <returns></returns>
    public static IEnumerable<GetUserOrganisationServicesResponse> ToUserDtos(this IEnumerable<UserOrganisationServicesQuery> models)
    {
        return models
            .GroupBy(x => x.UserId)
            .Select(userGroup => {
                UserOrganisationServicesQuery u = userGroup.First();
    ...
    this IGrouping<Guid, UserOrganisationServicesQuery> userGroup)
    {
        return userGroup
    ...
    this IGrouping<Guid, UserOrganisationServicesQuery> orgGroup)
    {
        return orgGroup
            .GroupBy(x => x.ServiceName ?? string.Empty)
            .OrderBy(x => x.Key)
            .Select(serviceGroup => {
                UserOrganisationServicesQuery s = serviceGroup.First();
    ...
    public static IEnumerable<RoleDto> ToRoleDtos(
        this IGrouping<string, UserOrganisationServicesQuery> serviceGroup)
    {
        return serviceGroup
