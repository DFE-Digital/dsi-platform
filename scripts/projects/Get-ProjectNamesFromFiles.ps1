...
function Get-ProjectNamesFromFiles {
    ...
    $testHelperProjects = @(Get-ChildItem -Directory "$path/tests" | Select-Object -ExpandProperty Name| ForEach-Object {
        if ($_ -notmatch '\.UnitTests$') { $_ }
    })
    $testProjects = ($associatedTestProjects + $changedTestProjects + $testHelperProjects) | Sort-Object -Unique
