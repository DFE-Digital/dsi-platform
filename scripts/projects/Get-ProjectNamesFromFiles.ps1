...
function Get-ProjectNamesFromFiles {
<#
    .SYNOPSIS
...
#>
    ...
    [String[]]$testHelperProjects = Get-ChildItem -Directory "$Path/tests" | Select-Object -ExpandProperty Name | ForEach-Object {
        if ($_ -notmatch '\.UnitTests$') { $_ }
    }
    [String[]]$associatedTestProjects = $sourceProjects + $testHelperProjects | ForEach-Object {
        "${_}.UnitTests"
    }
    [String[]]$changedTestProjects = $Files | ForEach-Object {
        if ($_ -match "^tests/([^\\/]+.UnitTests)") { $matches[1] }
    }
    [String[]]$testProjects = ($associatedTestProjects + $changedTestProjects + $testHelperProjects) | Sort-Object -Unique
